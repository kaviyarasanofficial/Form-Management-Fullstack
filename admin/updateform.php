...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE formcliant SET Center_Code='$centerCode', First_Name='$firstName', Last_Name='$lastName', DOB='$dateOfBirth', Phone_Number='$phoneNumber', Address='$address', City='$city', State='$state', Zip='$zip', Income='$income', Social_Security='$socialSecurity', Email='$email', Bank_Name='$bankName', Account_Number='$accountNumber', IsAbleToSpend='$isAbleToSpend', IsInterested='$isInterested' WHERE sno='$formID'";
    ...
    $insertStmt = $conn->prepare("INSERT INTO updateformhistory (form_id, email) VALUES (?, ?)");

if ($insertStmt) {
    $insertStmt->bind_param("ss", $formsID, $useremail);
    $insertStmt->execute();
} else {
